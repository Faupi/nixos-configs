...
in with lib; {
  imports = [ ./config-kwin.nix ./config-klipper.nix ];
        ...
        klipper = {
          ...
          actions = {
            "Spotify link" = {
              automatic = true;
              regexp =
                "^https?://open\\.spotify\\.com/(track|album)/([0-9|a-z|A-Z]+)";
              commands = {
                "Play video" = {
                  command =
                    "curl https://api.song.link/v1-alpha.1/links/?url='%s' | jq -j '.linksByPlatform.youtube.url' | grep -Eo '^https://www.youtube.com/watch?v=[a-zA-Z0-9_-]{11}$$' | xargs mpv --profile=builtin-pseudo-gui --fs";
                  icon = "mpv";
                  output = "ignore";
                };
                "Copy YouTube link" = {
                  command =
                    "curl https://api.song.link/v1-alpha.1/links/?url='%s' | jq -j '.linksByPlatform.youtube.url' | grep -Eo '^https://www.youtube.com/watch\\?v=[a-zA-Z0-9_-]{11}$$'";
                  icon = "youtube";
                  output = "replace";
                };
                "Copy SongLink link" = {
                  command =
                    "curl https://api.song.link/v1-alpha.1/links/?url='%s' | jq -j '.pageUrl'";
            ...
            "SongLink link" = {
              ...
              commands = {
                "Open in Spotify" = {
                  command =
                    "curl https://api.song.link/v1-alpha.1/links/?url='%s' | jq -j '.linksByPlatform.spotify.nativeAppUriDesktop' | xargs sh -c 'spotify --uri=$$1' sh";
                  icon = "spotify";
                  output = "ignore";
                };
                "Open in Spotify (better)" = {
                  command = ''
                    curl -s '%s' | htmlq --text '#__NEXT_DATA__' | jq -j '.props.pageProps.pageData.sections[] | select(.sectionId | test("links")?) | .links[] | select(.platform=="spotify") | .nativeAppUriDesktop' | xargs sh -c 'spotify --uri=$$1' sh'';
                  icon = "spotify";
                  output = "ignore";
                };
              };
