      ...
      actions =
        ...
        in
        {
          "Spotify link" = {
            ...
            commands = {
              "Play video" = {
                command = "${curl} -s https://api.song.link/v1-alpha.1/links/?url='%s' | ${jq} -j '.linksByPlatform.youtube.url' | ${grep} -Eo '^https://www.youtube.com/watch?v=[a-zA-Z0-9_-]{11}$$' | xargs mpv --profile=builtin-pseudo-gui --fs";
                icon = "mpv";
                output = "ignore";
              };
              "Copy YouTube link" = {
                command = "${curl} -s https://api.song.link/v1-alpha.1/links/?url='%s' | ${jq} -j '.linksByPlatform.youtube.url' | ${grep} -Eo '^https://www.youtube.com/watch?v=[a-zA-Z0-9_-]{11}$$'";
                icon = "youtube";
                output = "replace";
              };
              "Copy SongLink link" = {
                command = "${curl} -s https://api.song.link/v1-alpha.1/links/?url='%s' | ${jq} -j '.pageUrl'";
          ...
          "SongLink link" = {
            ...
            commands = {
              "Open in Spotify" = {
                command = "${curl} -s https://api.song.link/v1-alpha.1/links/?url='%s' | ${jq} -j '.linksByPlatform.spotify.nativeAppUriDesktop' | xargs sh -c 'spotify --uri=$$1' sh";
                icon = "spotify";
                output = "ignore";
              };
